    ...
    public byte[] data() {
        switch (getDataState()) {
            ...
            case BYTE_ARRAY_ONLY:
                return theData;
            case CHANNEL_ONLY:
                logger.info("ZIPPLACE: Creating array", new Throwable("DEBUG"));
                try (final SeekableByteChannel sbc = this.seekableByteChannelFactory.create()) {
                    final ByteBuffer buff = ByteBuffer.allocate(determineByteBufferSizeForChannel(this));
                    sbc.read(buff);
                    return buff.array();
                } catch (final IOException ioe) {
                    logger.error("Error when fetching from byte channel factory on object {}", shortName(), ioe);
                    return new byte[0];
                }
            case NO_DATA:
            default:
                return null; // maintains backwards compatibility
        }
    }

    /**
     * Work out the appropriate size for a byte array container e.g. a {@link ByteBuffer} given a BDO. Will log a warning if
     * the data would end up being truncated because of the size.
     * 
     * @param bdo to work out the size on
     * @return proposed size of the container
     * @throws IOException if size can't be obtained
     */
    public static int determineByteBufferSizeForChannel(final BaseDataObject bdo) throws IOException {
        final long channelSize = bdo.getChannelSize();
        final long truncatedBy = channelSize - Integer.MAX_VALUE;
        if (truncatedBy > 0) {
